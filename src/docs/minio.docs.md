...
```js
const arnFromMcCli =
  "arn:minio:replication::b22d653b-e4fb-4c5d-8140-7694c8e72ed4:dest-bucket";
...
```js
const replicatinConfig = await minioClient.getBucketReplication(
  "source-bucket"
);
...
```js
await minioClient.setObjectLockConfig("my-bucketname", {
  mode: "COMPLIANCE",
  unit: "Days",
  validity: 10,
});
...
const objectsStream = s3Client.listObjects(bucket, prefix, recursive, {
  IncludeVersion: true,
});
objectsStream.on("data", function (obj) {
...
```js
const retentionInfo = await minioClient.getObjectRetention(
  "bucketname",
  "objectname"
);
...
```js
const retInfoForVersionId = await minioClient.getObjectRetention(
  "bucketname",
  "objectname",
  {
    versionId: "my-versionId",
...
```js
console.log(
  await minioClient.getObjectTagging("bucketname", "object-name", {
    versionId: "my-object-version-id",
  })
);
```
...
```js
const legalholdStatus = await minioClient.getObjectLegalHold(
  "bucketName",
  "objectName"
);
```
...
```js
const legalholdStatus = await minioClient.setObjectLegalHold(
  "bucketName",
  "objectName",
  { Status: "ON" }
);
...
const res = await minioClient.selectObjectContent(
  "bucketName",
  "objectName",
  selectOpts
);
...
await minioClient.presignedUrl("GET", "mybucket", "", 1000, {
  prefix: "data",
  "max-keys": 1000,
});
```
...
```js
const listener = minioClient.listenBucketNotification(
  "my-bucketname",
  "photos/",
  ".jpg",
  ["s3:ObjectCreated:*"]
